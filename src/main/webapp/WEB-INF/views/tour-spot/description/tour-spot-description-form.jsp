...
<c:set var="pageTitle"
       value="${tourSpotDescriptionVm.id == null ? '여행지설명' : tourSpotDescriptionVm.name} ${tourSpotDescriptionVm.id == null ? ' 추가하기' : ' 편집하기'}"/>
<c:set var="subHeadingLabel"
       value="${tourSpotDescriptionVm.id != null ? tourSpotDescriptionVm.description :  '우리나라에 엄청난 관광객들이 몰려오고 있는 글로벌한 시대이다. 과거에는 부산이나 서울을 제외하고는 외국인 관광객들이 거의 관심을 두지 않았는데 시대가 바뀌고 지방의 여행지설명들도 좋은 곳들이 많아짐에 따라 전국에 외국인 관광객의 발길이 닿지 않는 곳이 없을 정도다.'}"/>
    ...
    <jsp:body>
        ...
        <div class="layout__half">
            <div class="path-bar">
                ...
                <c:if test="${tourSpotDescriptionVm.id != null}">
                    <t:pathBarDirectory url="/tour-spot/description/detail/${tourSpotDescriptionVm.id}"
                                        name="${tourSpotDescriptionVm.name} 상세보기"/>
                </c:if>
                <span>${pageTitle}</span>
            </div>
            ...
            <form:form action="${pageContext.request.contextPath}/tour-spot/description/save" modelAttribute="tourSpotDescriptionVm">

                <form:hidden path="id"/>
                <form:hidden path="tourSpotId"/>

                <div class="section__content">
                    <t:sectionSubHeading subHeading="여행지설명 기본정보" subHeadingLabel="여행지설명 기본정보를 입력해주세요"/>
                    ...
                    <div class="border-box">
                        ...
                        <t:contentWhiteBoxCheckbox label="활성화" value="${tourSpotDescriptionVm.enabled}"
                                                   fieldName="enabled"/>
                        ...
                        <t:contentWhiteBoxCheckbox label="팝업" value="${tourSpotDescriptionVm.popup}"
                                                   fieldName="popup"/>
