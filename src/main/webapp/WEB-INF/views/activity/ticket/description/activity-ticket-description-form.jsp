...
<c:set var="pageTitle"
       value="${activityTicketDescriptionVm.id == null ? '액티비티티켓설명' : activityTicketDescriptionVm.name} ${activityTicketDescriptionVm.id == null ? ' 추가하기' : ' 편집하기'}"/>
<c:set var="subHeadingLabel"
       value="${activityTicketDescriptionVm.id != null ? activityTicketDescriptionVm.description :  '우리나라에 엄청난 관광객들이 몰려오고 있는 글로벌한 시대이다. 과거에는 부산이나 서울을 제외하고는 외국인 관광객들이 거의 관심을 두지 않았는데 시대가 바뀌고 지방의 액티비티티켓설명들도 좋은 곳들이 많아짐에 따라 전국에 외국인 관광객의 발길이 닿지 않는 곳이 없을 정도다.'}"/>
...
<t:masterPage>
    ...
    <jsp:body>
        ...
        <div class="layout__half">
            <div class="path-bar">
                <t:pathBarDirectory url="/tour-spot/list" name="액티비티 리스트"/>
                ...
                <c:if test="${activityTicketDescriptionVm.id != null}">
                    <t:pathBarDirectory url="/tour-spot/ticket/description/detail/${activityTicketDescriptionVm.id}"
                                        name="${activityTicketDescriptionVm.name} 상세보기"/>
                </c:if>
                <span>${pageTitle}</span>
            </div>
            ...
            <form:form action="${pageContext.request.contextPath}/activity/ticket/description/save" modelAttribute="activityTicketDescriptionVm">

                <form:hidden path="id"/>
                <form:hidden path="activityTicketId"/>

                <div class="section__content">
                    <t:sectionSubHeading subHeading="액티비티티켓설명 기본정보" subHeadingLabel="액티비티티켓설명 기본정보를 입력해주세요"/>
                    ...
                    <div class="border-box">
                        ...
                        <t:contentWhiteBoxCheckbox label="활성화" value="${activityTicketDescriptionVm.enabled}"
                                                   fieldName="enabled"/>
                        ...
                        <t:contentWhiteBoxCheckbox label="팝업" value="${activityTicketDescriptionVm.popup}"
                                                   fieldName="popup"/>
